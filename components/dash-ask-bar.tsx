...
import { Sparkles, ArrowUp, Paperclip, X, FileText, Database, Globe, ChevronDown } from "lucide-react";
    ...
    <div className="w-full">
      ...
      {(pending.length > 0 || uploading) && (
        <div className="flex flex-wrap gap-1.5 mb-2">
          {pending.map((a) => (
            <div key={a.id} className="flex items-center gap-1.5 bg-[var(--surface-2)] border border-[var(--border)] rounded-lg pl-1.5 pr-1 py-1 max-w-[180px]">
              ...
              <button onClick={() => setPending((p) => p.filter((x) => x.id !== a.id))} className="p-0.5 text-[var(--faint)] hover:text-[var(--c-rose)]"><X className="w-3 h-3" /></button>
            </div>
          ))}
      ...
      {/* input row — charcoal field with a monochrome send button */}
      <div className="card flex items-center gap-2 pl-4 pr-2 py-2 focus-within:border-[var(--border-2)] transition-colors">
        <Sparkles className="w-4 h-4 text-[var(--muted)] shrink-0" strokeWidth={1.9} />
        <input
          value={ask}
        ...
        />
        <button onClick={askAI} disabled={!ask.trim() && pending.length === 0} title="Ask" className="btn-primary shrink-0 w-8 h-8 rounded-full">
      ...
      <div className="mt-2.5 flex items-center gap-1.5 flex-wrap">
        <div className="relative">
          <select
            ...
            className="pill appearance-none pr-7 cursor-pointer max-w-[190px] truncate text-[var(--text)]"
          >
          ...
          <ChevronDown className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--faint)]" />
        </div>

        <button onClick={() => fileRef.current?.click()} title="Attach files" className="pill">
          <Paperclip className="w-3.5 h-3.5" /> Files
        </button>

        <button onClick={() => setUseData((v) => !v)} title="Give the AI access to your Bridge data" className="pill" data-on={useData}>
          <Database className="w-3.5 h-3.5" /> Data
        </button>

        {webApplicable && (
          <button onClick={toggleWeb} title="Web search" className="pill" data-on={webOn}>
