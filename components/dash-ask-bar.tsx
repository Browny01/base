...
export function DashAskBar() {
  ...
  // white-alpha glass — the bar always sits on the dark graphite hero
  const chip = "flex items-center gap-1.5 h-8 px-2.5 rounded-full border text-[12px] font-medium transition-colors";
  const chipOff = "border-white/15 bg-white/10 text-white/70 hover:text-white hover:border-white/25";
  const chipOn = "border-white/40 bg-white/25 text-white";
    ...
    <div className="mt-5 mx-auto max-w-xl">
      ...
      {(pending.length > 0 || uploading) && (
        <div className="flex flex-wrap justify-center gap-1.5 mb-2">
          {pending.map((a) => (
            <div key={a.id} className="flex items-center gap-1.5 bg-white/10 border border-white/15 rounded-lg pl-1.5 pr-1 py-1 max-w-[180px]">
              {a.kind === "image" && a.url ? <img src={a.url} alt="" className="w-5 h-5 rounded object-cover" /> : <FileText className="w-3.5 h-3.5 text-white/60 shrink-0" />}
              <span className="text-[11px] text-white/90 truncate">{a.name}</span>
              <button onClick={() => setPending((p) => p.filter((x) => x.id !== a.id))} className="p-0.5 text-white/50 hover:text-red-400"><X className="w-3 h-3" /></button>
            </div>
          ))}
          {uploading && <span className="text-[11px] text-white/50 self-center px-1">Uploading…</span>}
        </div>
      )}

      {/* input pill — liquid glass on the graphite hero */}
      <div className="lg-pill flex items-center gap-1.5 rounded-full pl-4 pr-1.5 py-1.5 focus-within:border-white/35 transition-colors">
        <Sparkles className="w-4 h-4 text-white/60 shrink-0" strokeWidth={2} />
        <input
          value={ask}
        ...
        />
        <button onClick={askAI} disabled={!ask.trim() && pending.length === 0} title="Ask" className="lg-btn-metal shrink-0 w-8 h-8 flex items-center justify-center rounded-full disabled:opacity-40">
      ...
      <div className="mt-2.5 flex items-center justify-center gap-1.5 flex-wrap">
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none h-8 pl-2.5 pr-7 rounded-full border border-white/15 bg-white/10 text-[12px] font-medium text-white/80 focus:outline-none focus:border-white/35 cursor-pointer max-w-[190px] truncate"
          >
          ...
          <ChevronDown className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/50" />
        </div>

        <button onClick={() => fileRef.current?.click()} title="Attach files" className={cn(chip, chipOff)}>
          <Paperclip className="w-3.5 h-3.5" /> Files
        </button>

        <button onClick={() => setUseData((v) => !v)} title="Give the AI access to your Bridge data" className={cn(chip, useData ? chipOn : chipOff)}>
          <Database className="w-3.5 h-3.5" /> Data
        </button>

        {webApplicable && (
          <button onClick={toggleWeb} title="Web search" className={cn(chip, webOn ? chipOn : chipOff)}>
